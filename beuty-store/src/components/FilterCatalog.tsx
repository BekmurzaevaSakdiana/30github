...
const FilterCatalog = ({ params, searchParams, name }: FilterCatalogProps) => {
  ...
  const handleFilterChange = (key: string, value: string, isMultiSelect = false) => {
    const current = new URLSearchParams(search.toString());
    ...
    <div className="catalogItems mt-10">
      <div className="aside__products">
        <aside className="max-w-60 w-full border-r-2 max-xl:border-none max-xl:max-w-32">
          ...
          <form className="filter-form" onChange={(e) => e.preventDefault()}>
            <div className="typeof">
              <p className="font-bold text-lg mt-8">{name}</p>
              {loading ? <p>Загрузка...</p> : subCategory.map((item) => (
                <label key={item.id} className="block">
                  <input
                    type="checkbox"
                    checked={search.getAll("subcategory").includes(item.id)}
                    onChange={() => handleFilterChange("subcategory", item.id, true)}
                  />
                  {item.name}
                </label>
              ))}
            ...
            <div className="brands">
              <p className="font-bold text-lg mt-8">Бренд</p>
              {loading ? <p>Загрузка...</p> : allBrands?.results.map((brand) => (
                <label key={brand.name} className="block">
                  <input
                    type="checkbox"
                    checked={search.getAll("brand").includes(brand.name)}
                    onChange={() => handleFilterChange("brand", brand.name, true)}
                  />
                  {brand.name}
                </label>
              ))}
          ...
          <button onClick={() => router.push("?")} className="mt-12 text-black hover:text-red-600">Очистить</button>
        </aside>
      </div>
    </div>
  );
